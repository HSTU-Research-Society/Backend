...
export default function TermsOfServicePage() {
  ...
  useEffect(() => {
    async function fetchContent() {
      try {
        // Check primary and fallback document IDs for compatibility
        let docSnap = await getDoc(doc(db, "pages_static", "terms_of_service"));
        if (!docSnap.exists()) {
          docSnap = await getDoc(doc(db, "pages_static", "terms"));
        }
        if (!docSnap.exists()) {
          docSnap = await getDoc(doc(db, "pages_static", "terms-of-service"));
        }

        if (docSnap.exists()) {
          const data = docSnap.data();
          setContentMarkdown(data.contentMarkdown || data.content || data.markdown || "");
        }
      } catch (err: any) {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      // Save to standard keys so all frontend variants can read it
      await Promise.all([
        setDoc(doc(db, "pages_static", "terms_of_service"), payload, { merge: true }),
        setDoc(doc(db, "pages_static", "terms"), payload, { merge: true }),
        setDoc(doc(db, "pages_static", "terms-of-service"), payload, { merge: true })
      ]);
        ...
        {loading ? (
          ...
          <>
            <textarea
              ...
              placeholder="Write Terms of Service in Markdown format..."
