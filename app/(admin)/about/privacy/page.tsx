...
export default function PrivacyPolicyPage() {
  ...
  useEffect(() => {
    async function fetchContent() {
      try {
        ...
        let docSnap = await getDoc(doc(db, "pages_static", "privacy_policy"));
        if (!docSnap.exists()) {
          docSnap = await getDoc(doc(db, "pages_static", "privacy"));
        }
        if (!docSnap.exists()) {
          docSnap = await getDoc(doc(db, "pages_static", "privacy-policy"));
        }
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      await Promise.all([
        setDoc(doc(db, "pages_static", "privacy_policy"), payload, { merge: true }),
        setDoc(doc(db, "pages_static", "privacy"), payload, { merge: true }),
        setDoc(doc(db, "pages_static", "privacy-policy"), payload, { merge: true })
      ]);
        ...
        {loading ? (
          ...
          <>
            <textarea
              ...
              placeholder="Write Privacy Policy in Markdown format..."
